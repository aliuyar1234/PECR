...
if [[ -z "${CONTROLLER_BASELINE_EXPECT_TERMINAL_MODE}" ]]; then
  case "${PECR_CONTROLLER_ENGINE_OVERRIDE,,}" in
    rlm)
      CONTROLLER_BASELINE_EXPECT_TERMINAL_MODE="SUPPORTED"
      ;;
    *)
      CONTROLLER_BASELINE_EXPECT_TERMINAL_MODE="INSUFFICIENT_EVIDENCE"
      ;;
  esac
fi
...
run_k6_controller_baseline() {
  docker_compose --profile perf run --rm -T \
    ...
    -e LOCAL_AUTH_SECRET="${PECR_LOCAL_AUTH_SHARED_SECRET}" \
    -e QUERY="smoke" \
    -e EXPECT_TERMINAL_MODE="${CONTROLLER_BASELINE_EXPECT_TERMINAL_MODE}" \
    -e ENFORCE_P99="1" \
    -e P99_BUDGET_MS="$CONTROLLER_P99_BUDGET_MS" \
    -e VUS="$BASELINE_VUS" \
