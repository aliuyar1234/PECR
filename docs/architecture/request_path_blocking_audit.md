...
| Controller RLM bridge (`/v1/run`) | Per-request Python bridge spawn added steady latency and jitter to every RLM request | Controller now reuses a cached persistent bridge worker and only respawns on protocol/process failure | One persistent worker currently serializes RLM bridge traffic per controller process; this removes spawn overhead first, then parallel pools can follow |
