-- PECR V1 baseline ledger schema + append-only enforcement.
--
...
CREATE TABLE IF NOT EXISTS pecr_policy_snapshots (
...
-- 2) Sessions
CREATE TABLE IF NOT EXISTS pecr_sessions (
...
CREATE TABLE IF NOT EXISTS pecr_ledger_events (
...
CREATE INDEX IF NOT EXISTS pecr_ledger_events_trace_id_idx ON pecr_ledger_events(trace_id);
CREATE INDEX IF NOT EXISTS pecr_ledger_events_event_type_idx ON pecr_ledger_events(event_type);

-- 4) Evidence units (metadata; payload optional)
CREATE TABLE IF NOT EXISTS pecr_evidence_units (
...
CREATE INDEX IF NOT EXISTS pecr_evidence_units_trace_id_idx ON pecr_evidence_units(trace_id);
CREATE INDEX IF NOT EXISTS pecr_evidence_units_object_id_idx ON pecr_evidence_units(object_id);

-- 5) Claim maps (finalization records)
CREATE TABLE IF NOT EXISTS pecr_claim_maps (
