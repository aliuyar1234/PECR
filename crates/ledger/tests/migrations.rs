...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn session_runtime_rejects_bytes_used_above_bigint_range() {
    let Some(db_url) = test_db_url() else {
        eprintln!("skipping session runtime overflow test; set PECR_TEST_DB_URL to enable");
        return;
    };
    ...
    let err = writer
        .upsert_session_runtime(pecr_ledger::SessionRuntimeWrite {
            ...
            session_token_expires_at_epoch_ms: 1_000_000,
            operator_calls_used: 2,
            bytes_used: i64::MAX as u64 + 1,
            evidence_unit_ids: &[],
            finalized: false,
        })
        .await
        .expect_err("oversized bytes_used should be rejected");
